...
describe("run", () => {
  ...
  it("calls the error branch with the error", async () => {
    ...
    const error: unknown = reject.mock.calls[0]?.[0];
  ...
  describe("when all awaited values in the block are successful", () => {
    it("returns the returned value as an async", async () => {
      const result$ = Future.block(async ($) => {
        ...
        return value1 + Number.parseInt(value2) + value3;
      });
  ...
  describe("when any the awaited values in the block is an error", () => {
    it("returns that error as the async result", async () => {
      const result$ = Future.block<string, number>(async ($) => {
        const value1 = await $(Future.success(1));
        const value2 = await $(Future.error<string, number>("message"));
        const value3 = await $(Future.success(3));
...
describe("fromComputation", () => {
  describe("for a successful computation", () => {
    it("return a success async", async () => {
      const value$ = Future.fromComputation((resolve) => {
        resolve(1);
        return () => {};
      });
  ...
  describe("for an unsuccessful computation", () => {
    it("return an error async", async () => {
      const value$ = Future.fromComputation<Error, string>((_resolve, reject) => {
    ...
    it("preserves typed non-Error rejections", async () => {
      const value$ = Future.fromComputation<string, number>((_resolve, reject) => {
        reject("typed-error");
        return () => {};
      });

      await expectAsync(value$, { toThrow: "typed-error" });
    });
  });

  describe("when canceled", () => {
    it("does not call the success or error branch even if the computation later resolves", async () => {
      const success = vi.fn();
      const reject = vi.fn();

      const value$ = Future.fromComputation<Error, number>((resolve) => {
        setTimeout(() => {
          resolve(1);
        }, 1);
        return undefined;
      ...
      const cancel = value$.run(success, reject);
      cancel?.();
      await sleep(10);
...
describe("joinObj", () => {
  ...
  it("returns an error if some of the inputs is an error", async () => {
    const join$ = Future.joinObj({
      n: Future.success<string, number>(123),
      s: Future.error<string, unknown>("Some error"),
    });
    expectTypeOf(join$).toEqualTypeOf<Future<unknown, { n: number; s: unknown }>>();
...
export async function expectAsync<E, D>(
  ...
  if ("toEqual" in options) {
    ...
    await expect(value$.toPromise()).rejects.toEqual(options.toThrow);
  }
}
