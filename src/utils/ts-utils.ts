...
export type UnionFromValues<T extends readonly unknown[]> = T[number];
...
export function isValueInUnionType<S, T extends S>(value: S, values: readonly T[]): value is T {
  return (values as readonly S[]).includes(value);
}
...
export function recordOf<T>() {
  return function <Obj extends Record<string, T>>(obj: Obj): Obj {
    return obj;
  };
}
...
export function assertUnreachable(value: never, message?: string) {
  ...
  const finalMessage = message ?? `Invalid union value: ${reason}`;
