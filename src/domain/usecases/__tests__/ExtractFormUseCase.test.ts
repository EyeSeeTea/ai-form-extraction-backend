    ...
    await expect(useCase.execute(extractFormInput).toPromise()).resolves.toMatchObject(
      createExtractFormResult({
        ...
        diagnostics: {
          ...
          issues: [
            {
              path: "/end_of_season_report/header_information/team",
              code: "required",
              message: "Missing field: end_of_season_report.header_information.team",
            },
            {
              path: "/end_of_season_report/header_information",
              code: "custom",
